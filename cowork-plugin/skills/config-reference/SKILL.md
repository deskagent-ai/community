...
> **⚠️ HINWEIS:** `agents.json` ist der **alte Weg**. Der empfohlene Weg ist die Konfiguration direkt im **JSON-Frontmatter der .md Datei**.
>
> Siehe **[doc-agent-frontmatter-reference.md](doc-agent-frontmatter-reference.md)** für die vollständige Referenz.

Diese Datei dient nur noch als Fallback/Override für Agents ohne Frontmatter.
...
### Empfohlener Weg: Frontmatter in .md Dateien

Agents werden in Markdown-Dateien (`agents/*.md`) mit JSON-Frontmatter definiert:
...
**Vorteile:**
- Alles in einer Datei (Konfiguration + Prompt)
- Einfacher zu versionieren
- Keine Synchronisation zwischen Dateien nötig

### Merge-Priorität

```
1. Agent .md Frontmatter       ← Höchste Priorität (empfohlen!)
...
Falls doch benötigt (z.B. für Skills ohne eigene .md Datei):
