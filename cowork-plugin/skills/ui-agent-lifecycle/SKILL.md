...
**KRITISCH:** Der komplette Konversationskontext MUSS während des gesamten Agent-Lebenszyklus erhalten bleiben. Dies ist eine fundamentale Anforderung für alle AI-Backend-Implementierungen.

**Scope:** Diese Anforderung gilt **pro Agent-Session**. Jeder neue Agent-Start beginnt mit frischem Kontext. Der Kontext wird NICHT zwischen verschiedenen Agenten geteilt.

### Wann Context erhalten bleiben MUSS

| Situation | Anforderung |
|-----------|-------------|
| **Nach CONFIRMATION_NEEDED** | Agent erhält alle vorherigen Turns + User-Bestätigung |
| **Nach QUESTION_NEEDED** | Agent erhält alle vorherigen Turns + User-Antwort |
| **Bei User Follow-up Prompt** | Agent erhält komplette bisherige Konversation |
| **Bei Session-Fortsetzung (History)** | Agent erhält alle gespeicherten Turns der Session |
| **Nach Tool-Aufrufen** | Tool-Ergebnisse bleiben im Kontext für Folge-Reasoning |

### Was zum Context gehört

```
Conversation Context (an AI-Backend gesendet)
├── System Prompt (Agent-Instruktionen)
├── Knowledge (geladene Wissensbasis)
├── Turn 1: User Prompt
├── Turn 2: Assistant Response
│   ├── Tool Calls (Name, Parameter)
│   └── Tool Results (Rückgabewerte)
├── Turn 3: User Input (Dialog-Antwort oder Follow-up)
...
**Backend-Implementierungen müssen:**

1. **Conversation History speichern** - Alle Turns (User + Assistant) in einer Liste
2. **Tool-Ergebnisse einbinden** - Tool-Calls und deren Results als Teil der Assistant-Turns
3. **Bei Dialog-Antwort fortsetzen** - User-Input als neuen Turn anhängen, nicht neu starten
4. **Bei Session-Continue laden** - Gespeicherte Turns aus `session_store` wiederherstellen

**Anti-Pattern (FALSCH):**
```python
# ❌ FALSCH: Kontext geht verloren
def handle_dialog_response(response):
    # Startet neue Konversation - verliert allen Kontext!
    return ai_client.chat([{"role": "user", "content": response}])
```

**Korrekte Implementierung:**
```python
# ✅ RICHTIG: Kontext bleibt erhalten
def handle_dialog_response(response, conversation_history):
    # Fügt User-Antwort zur bestehenden Historie hinzu
...
|------------|---------------|
| `AgentTask` | Hält `conversation_history` während Task-Ausführung |
| `session_store.py` | Persistiert Turns in SQLite für Session-Fortsetzung |
| AI-Backend (claude_sdk, gemini, etc.) | Sendet komplette History bei jedem API-Call |
| `routes/execution.py` | Lädt Session-Context bei Continue/Dialog-Response |

### Verifikation

Bei Problemen mit Context-Verlust prüfen:

1. **Agent-Log** (`workspace/.logs/agent_latest.txt`) - Zeigt gesendete Messages
2. **Session-Store** - `GET /session/{id}/context` liefert gespeicherte Turns
3. **SSE-Events** - `content_sync` Event enthält aktuellen Konversationsstand
