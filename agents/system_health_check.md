---
{
  "name": "System-Prüfung",
  "category": "system",
  "description": "Analysiert System-Logs und prueft den Gesundheitszustand",
  "icon": "monitor_heart",
  "input": ":assignment: System-Logs",
  "output": ":analytics: Health-Report",
...
1. **Agent-Log pruefen** (`{{LOGS_DIR}}/agent_latest.txt`)
   - Lies die Datei mit `desk_get_agent_log()` oder `fs_read_file("{{LOGS_DIR}}/agent_latest.txt")`
   - Falls nicht vorhanden: Informiere dass noch kein Agent gelaufen ist

2. **System-Log pruefen** (`{{LOGS_DIR}}/system.log`)
   - Lies die letzten 500 Zeilen mit `fs_read_file("{{LOGS_DIR}}/system.log")`
...
--- Agent Log ---
Datei: {{LOGS_DIR}}/agent_latest.txt
Agent: [Name]
Task: [Task]
Erfolg: [Ja/Nein]
