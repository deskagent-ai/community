...
{
  "category": "system",
  "description": "Fuegt neues Wissen intelligent zur Knowledge Base hinzu",
  "icon": "library_add",
  "input": ":upload_file: Dateien oder Text",
  "output": ":library_books: Aktualisierte Knowledge Base",
  "allowed_mcp": "filesystem|clipboard",
  "tool_mode": "write_safe",
  "order": 90,
  "enabled": true,
  "anonymize": false,
  "inputs": [
    {"name": "files", "type": "file", "label": "Dateien (optional)", "required": false, "multiple": true, "accept": ".md,.txt,.pdf"},
    {"name": "content", "type": "text", "label": "Text-Input (optional)", "required": false, "multiline": true, "rows": 8, "placeholder": "Neues Wissen hier eingeben oder aus Clipboard..."}
  ]
}
---
...
```
fs_list_all_files("{{KNOWLEDGE_DIR}}", "**/*.md")
```
...
```
fs_read_file("{{KNOWLEDGE_DIR}}/<relativer_pfad>")
```
...
```
fs_read_file("{{KNOWLEDGE_DIR}}/[datei].md")
```
...
```
fs_write_file("{{KNOWLEDGE_DIR}}/[datei].md", "[kompletter neuer Inhalt]")
```
...
2. **Datei erstellen:**
```
fs_write_file("{{KNOWLEDGE_DIR}}/[neue_datei].md", "[strukturierter Inhalt]")
```
